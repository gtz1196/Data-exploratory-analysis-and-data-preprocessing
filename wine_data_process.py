...
# 标称属性，生成直方图
def nomial_process(column, figname, head):
    frequency = {}
    for i in column:
...
# 数值属性，生成盒图
def value_process(column, figname, head):
    ls = []
    sum = 0
    for i in column:
        if i == 'None':
...
if __name__ == '__main__':
    Nominal = ['country', 'designation', 'province', 'region_1', 'region_2', 'taster_name']
    Value = ['points']
    # 数据文件路径
    data_path = 'freq-winemag-data-130k-v2.csv'
    head = data_path[:4]+' '
    # 清洗，把空值置为None
    ...
    for i in data.keys():
        ...
        # 对于标称属性，生成直方图
        if i in Nominal:
            nomial_process(column, i, head)
        # 对于数值属性，生成盒图
        elif i in Value:
            value_process(column, i, head)
    with open(head+'abstract'+'.json', 'w') as f:
